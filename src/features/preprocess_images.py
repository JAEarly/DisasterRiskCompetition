    ...
    for country, regions in utils.LOCATIONS.items():
        for region in regions:
            ...
            if os.path.exists(competition_path):
                for image_name in tqdm(os.listdir(competition_path),
                                       desc=country + " " + region + " test",
                                       leave=False):
                    image_input_path = os.path.join(competition_path, image_name)
                    image_output_path = os.path.join(COMPETITION_OUTPUT_PATH, image_name)
                    shutil.copyfile(image_input_path, image_output_path)
            time.sleep(1)
...
def validate_dataset():
    ...
    print("      Train:", num_train_images, " Split:", "{:.3f}".format(num_train_images/num_total_images))
    print(" Validation: ", num_validation_images, " Split:", "{:.3f}".format(num_validation_images/num_total_images))
    print("       Test: ", num_test_images, " Split:", "{:.3f}".format(num_test_images/num_total_images))
    print("      Total:", num_total_images)

    assert num_train_images + num_validation_images + num_test_images == num_total_images
    ...
    for category, path in [("Training", TRAIN_OUTPUT_PATH),
